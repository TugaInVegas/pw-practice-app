...
test('sliders', async ({ page }) => {
    ...
    // Mouse movement
    const tempArea = page.locator('[tabtitle="Temperature"] ngx-temperature-dragger')
    await tempArea.scrollIntoViewIfNeeded()

    const box = await tempArea.boundingBox()
    ...
    await page.mouse.up()
    await expect(tempArea).toContainText('30')
})